...
function WrtieToTSConfig(filePath) {
    fs.readFile(filePath, "utf-8", (err, data) => {
        ...
        if (isModified) {
            let updatedData = JSON.stringify(config, null, 2);
            fs.writeFile(filePath, updatedData, 'utf8', (writeErr) => {
                if (writeErr) {
                    Log.error(`Error writing the updated file: ${writeErr}`);
                    return;
                }
                Log.success('File tsconfig.json has been updated successfully.');
            })
        } else {
            Log.warning('No updates were necessary.');
        }

    })
}

/** Install dotenv, create .env and add it to gitignore */
function dotenv() {
    fs.open('.env', 'w', function (err, file) {
        ...
        Log.success('.env file created succesfuly!');
    });
    ...
    if (fs.existsSync(gitignorePath)) {
        currentContent = fs.readFileSync(gitignorePath, { encoding: 'utf-8' });
    }
    ...
    if (!lines.has(content)) {
        ...
        Log.success(`Appended "${content}" to ${gitignorePath}`);
    } else {
        Log.warning(`"${content}" already exists in ${gitignorePath}`);
    }

}

// Function to get all folder names in a directory
function getAllFolderNames(directoryPath) {
    try {
        const entries = fs.readdirSync(directoryPath, { withFileTypes: true });
        const folderNames = entries.filter(entry => entry.isDirectory()).map(dir => dir.name);
        return folderNames;
    } catch (error) {
...
// Example usage
const foldersPath = './src/webparts';
const webPartName = getAllFolderNames(foldersPath);

const directoryPath = `./src/webparts/${webPartName[0]}`; // Adjust the path to your directory
...
const dotenvCommands = [{ command: 'npm', args: ['i', 'dotenv'] }]

const pathToAssetFiles = "./../../AssetsFiles"

// Example usage:
// getManifestInDirectory(directoryPath);
// getMainWebPartFileInDirectory(directoryPath, filePattern);
// deleteEslintrc()
// copyFileOrDirectory(`${pathToAssetFiles}/PnPjsConfig.ts`, "./src/webparts")
// copyFileOrDirectory(`${pathToAssetFiles}/services`, "./services")
// copyFileOrDirectory(`${pathToAssetFiles}/fast-serve`, "./fast-serve")
// copyFileOrDirectory(`${pathToAssetFiles}/createCmp.js`, "./")
...
// runCommandsWithInput(dotenvCommands)
// WrtieToTSConfig("./tsconfig.json")
// dotenv()
